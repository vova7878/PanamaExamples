...
public class CompositeLookupTest {
    ...
    public void testLookups(SymbolLookup lookup, List<Result> results) {
        for (Result result : results) {
            if (result instanceof Success test) {
                Optional<MemorySegment> symbol = lookup.find(test.name);
                assertTrue(symbol.isPresent());
                assertEquals(symbol.get().address(), test.expectedLookupId);
            } else if (result instanceof Failure test) {
                Optional<MemorySegment> symbol = lookup.find(test.name);
